...
Df  = 1e-4;                   key  = 2;        % Distance between f & tolerance
...
for i = 1:nh                  % h-counter
  ...
  for f = A:hf:B
     ...
     [d0 TE] = APSr(t,f,h,CN,aa,WImet,RPmet,WIop,RPop);  
     ...
     fprintf('%d %7.4f %8.1f %6.1g %6.1g  %d  %d  %d',c,Tab(c,:));
               % WImet(@RHFN,t0+[0 TE/2 TE],y000,WIop,aa,4)
     fifi(fr,h,CN);                                       warning('off','all');
     [t_,y,n_] = WImet(@F_DNK,t0+[0 TE/2 TE],y000,WIop,aa); warning( 'on','all');
