% Search for symmetric periodic oscillations in a DNA molecule
% f -> TE (in APSr) -> f (in fminsearch)
% Use: DY1(on T) + fminsearch
% Search for smooth dependence T(fi10)

function DNK_17                   % 17.07.2021 
global  a  b  y000  TE 
dati('DNK_17  Tr->fr');
...
Df  = 1e-3;                   % Distance between f & tolerance
e1  = 1e-4;   e2 = 1e-12;     EPS  = eps;             
inf = sprintf('%s %s CN=%d t9=%d hma=%5.3g er=%6.1g %6.1g EPS=%6.1g',...
              func2str(WImet),func2str(RPmet),CN,t9,hma,e1,e2,EPS); 
fprintf('%s\n',inf);  
                              % Main loop                         
for i = 1:nh                  % h-counter
  ...
  A  = acos(1-max(0,xmi))+Df;   B   = acos(1-min(1,xma))-Df; 
  Ao = A*opi;                   Bo  = B*opi; 
  fprintf('------ i=%d  h =%5.3g   %6.3g < fi10 <%5.3g\n',i,h,Ao,Bo);
  c = 0;                        hf  = 1e-2;
  for f = A:hf:B
     c  = c+1; 
     % By f we find TE in APSr
     [d0 Tr] = APSr(t,f,h,CN,aa,WImet,RPmet,WIop,RPop);  
     
     % By TE we clarify f in fminsearch
     [Trr,yr,eF,O] = fminsearch(@(T) DYT(T,CN,WImet,WIop,aa),Tr,...
                     optimset('TolX',EPS));  
     fc = O.funcCount;  % YE = d0<e1 && yr<e2;
     Tab(c,:) = [fr*opi TE d0 yr eF fc]; 
     fprintf('%d %7.4f %8.1f %6.1g %6.1g  %d  %d  %d',c,Tab(c,:));      
     
     TE = Trr;
     [fr,yr,eF,O] = fminsearch(@(F) DY1(F,h,CN,[0 TE/2 TE],WImet,WIop,aa),f,...
     optimset('TolX',EPS));  
     fc = O.funcCount;   YE = d0<e1 && yr<e2;
