...
% Comparison on half-period=T/2  ( by DY2 ) + fminsearch
...
for i = 1:nh                  % h-counter
  ...
  for f = [A+tolf (ceil(Ao):floor(Bo))/opi B-tolf]
     ...
     [fr,yr,eF,O] = fminsearch(@(F) DY2(F,h,CN,[t0 TE/2],WImet,WIop,aa),f,...
     optimset('TolX',EPS));  
