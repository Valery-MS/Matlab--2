...
function DNK_3
global yTA   % calcs in dop853t с.164:  Tj = fminbnd(@(t) -dop853t(t,... )

tic
NuMet = 2;
RelT  = 1e-14;  AbsT = 1e-14; 
h     = 1;   
...
u   = 10;    % degree step
ivd = [1:0.2:090 ];   %5:u:45 48 50:u:90];  % initial dev range - 1st pendulum: CN=1
%ivd = ...;   % initial deviation range - 2st pendulum: CN = 2
%iv = ...;    % initial velocity  range - 1st pendulum if CN = 3
%iv = ...;    % initial velocity  range - 2st pendulum if CN = 4
if CN <= 2, iv = ivd*pi/180; end       % range of fi-values(in radians)
qiv = numel(ivd);                      % qty of  initial values

t0 = 0;    tf = 6000;   
tA = 3200; tB = 6000;   tik = 500;
...
for nF = 1:qF 
   ...
   for s = 1:qB   % Blocks
      ...
      yi(1:Nopp) = NaN;        % Turn off value y0 from yi
                   % This block is replaced by the following block.
      ...
      while r<kT && j<q    
         ...
         % sign before dop853t = -sign(y0iC); Уточнение
         warning('off','all') % op853t(t,CN,tM,yM,F,op,Farg)
         Tj = fminbnd(@(t)    -dop853t(t,CN,tM,y(M,:)',@F_DNK,op,a),...
