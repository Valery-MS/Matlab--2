% Search for periodic oscillations in a DNA molecule
...
GRAF = 0;                 % grafs are plotting or no
...
WIN_ = 2:3;  % for Whole Interval (several thousand points):    1-3
RPN_ = 2:4;  % to Refine the Period: of 2-4 (the best is odex): 2-4
SPN_ = 2:3;  % to search for the Second Period:                 1-3
...
Figs = [];
nht=numel(ht);
TF=cell(nht,numel(WIN_)*numel(RPN_)*numel(SPN_));
for ih = 1:nht, h=ht(ih);Q=0;
for WIN=WIN_, for RPN=RPN_, for SPN=SPN_
if WIN == 1
...
fprintf('%s\nFfi=%d CN=%d ivd=%d:%d:%d RT0=%g tf=%d tA=%d tB=%d T2ma=%d\n',...
         FUNsh, Ffi, CN, ivd(1),ivd(2)-ivd(1),ivd(end),RT0,tf,tA,tB,T2ma);
...
qGB = qG*qB;
qF  = ceil(qiv/qB);     % quantity of Figures
...
i  = 0;               % ivd counter
                        ...
                        % Main LOOP
%for nF = 1:qF    % Figures
while true   %nF<qF*qvarFUN   
   if GRAF
      Figs = [Figs figure('Positi',SZ,'Name',FUNsh,'NumberTit','on')];
      nF = Figs(end).Number; end
   s = 1; 
   while s <= qB   % Blocks in Fugures
      i   = i+1; 
      fi0 = sprintf('%g',ivd(i));
      ...
      ay = abs(yi(Nopp+1:end)/ivi-1);
      m_ = find(ay<RT0);
      am = ay(m_);

      q = 0; x = 1; cs = 0;  I = m_;   % cs - cч соседей      
      nm = numel(m_);            % Исключаются соседние с минимумами точки -
      for j = 1:nm               % тот же min, но погешность больше
         if j < nm  &&  m_(j+1)-m_(j) == 1
            if am(j+1) < am(j), x = j+1;  end
            cs = cs+1; continue,end  
      ...
      while r < kT  &&  j < q    
         j = j+1;  M = Ms(j);   tM = t(M);
         if r > 0                      % search for a second (or more) period
            dr = tM./T(1:r,i);
         ...
         % sign before dop853t = -sign(y0iC); more accurate period value (Tj)
   ...
   if i == qiv, break,end,end  % for nF=1:qF - end of plotting of Figure № nF
